...
interface Stats { total: number; new: number; hot: number; contacted: number; reviewed: number; starred?: number; mock_search?: boolean; }
...
interface AgentContext {
  config: { agent_name: string; sender_name: string; sender_company: string; sender_description: string; qualifier_context: string; ideal_customer_profile?: string; what_we_offer?: string; product_focus?: string };
...
function dealDisplayName(lead: Lead): string {
  const p = (lead as Lead & { product_name?: string }).product_name;
  return (p || lead.brand_name || lead.company_name || lead.lead_name as string || "Unknown") as string;
}

/** True when the deal was indexed from mock Serper fallback (no API key) or synthetic no-link rows. */
function isPlaceholderDealUrl(url: string | undefined): boolean {
  if (!url) return false;
  return url.includes("example.com/mock") || url.includes("local.serper.placeholder");
}
...
/** Light-surface pills with dark label text — readable on the light Sales Deal dashboard */
...
function AddCompanyModal({
  ...
  const handleSubmit = async (forceAdd = false) => {
    ...
    setResult(null);
    try {
      const r = await fetch(`${API}/sales-deals/${projectId}/deals/manual`, {
        method: "POST",
          ...
          {!result && (
            <>
              <p className="text-xs text-ui-accent">
                Enter a product or merchant search. The agent will look it up, score it against your criteria, and add it to your deals if it qualifies.
              </p>
            ...
            <div className="space-y-3">
              ...
              {result.saved ? (
                <div className="flex items-center gap-2 text-xs text-green-700 font-medium">
                  <Check className="w-4 h-4" /> Added to your deals list
                </div>
              ) : (
                <div className="space-y-2">
                  ...
                  <p className="text-xs text-ui-accent">
                    You can still add it — manually added deals act as ICP examples that help refine future searches.
                  </p>
                  <button
...
function ProjectSelector({
  ...
  const handleCreate = async () => {
    ...
    setSaving(true);
    try {
      const r = await fetch(`${API}/sales-deals/projects`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      <div className="w-full max-w-2xl">
        ...
        <div className="mb-8">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h1 className="text-2xl font-semibold text-ui-text">Sales Deal Projects</h1>
          </div>
          <p className="text-sm text-ui-accent">
            Each project has its own deal pipeline, search context, and configuration.
...
function ManualSetupForm({
  ...
  const [form, setForm]     = useState({
    ...
    weak_signals: "",
    search_queries_raw: "",
    search_channels: ["google", "news", "reddit"] as string[],
    search_geo: "",
  ...
  const handleSave = async () => {
    ...
    if (!form.product_focus.trim() && !form.ideal_customer_profile.trim()) {
      setErr('Add "What deals you\'re looking for" (product focus) or region/buyer context so the agent knows what to match.');
      return;
    }
    setSaving(true);
    setErr("");
    ...
    const pf = form.product_focus.trim();
    const geo = form.search_geo.trim() || "India";
    const search_queries = rawQueries.length
      ? [{ signal: "general", queries: rawQueries }]
      : [{ signal: "general", queries: [
          `${pf || "product"} best price deal ${geo}`,
          `${pf || form.sender_company} discount sale online ${geo}`,
        ] }];

    const cfg = {
      agent_name:            form.agent_name || `${form.sender_company} Sales Deal Agent`,
      ...
      result_schema: {
        ...
        signal_types: ["price_drop", "clearance", "marketplace_promo"],
      },
    ...
    try {
      ...
      const r = await fetch(`${API}/sales-deals/${projectId}/config`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div className="h-full overflow-y-auto px-6 py-8 max-w-2xl mx-auto">
      ...
      <div className="space-y-4">
        ...
        {field("Agent name", "agent_name", "My Sales Deal Agent")}
        ...
        <div className="border-t border-ui-border pt-4">
          ...
          {field("What deals you’re looking for *", "product_focus",
            "e.g. Wireless earbuds under ₹2,500 in India — official store, retail chain, or trade news (not only big marketplaces)", true)}
          {field("Qualifier context *", "qualifier_context",
            "Short: you hunt retail/commerce snippets for real prices, discounts, and in-stock deals that match the product focus above.", true)}
          {field("Region / buyer context", "ideal_customer_profile",
            "e.g. India online + metro retail; field reps who need verified prices", true)}
          {field("What we track / do with hits", "what_we_offer",
            "e.g. Share best offers with the sales team same day", true)}
        </div>
        ...
        <div className="border-t border-ui-border pt-4">
          ...
          {field("Strong signals (one per line)", "strong_signals",
            "Explicit % off or lowest price in title/snippet\nOfficial store, authorised reseller, or press/trade coverage\nIn-stock / same-day delivery", true)}
          {field("Weak / ignore signals (one per line)", "weak_signals",
            "No price in snippet\nWrong country\nUsed/refurb only when you want new", true)}
        </div>
        ...
        <div className="border-t border-ui-border pt-4">
          ...
          {field("Search queries (one per line)", "search_queries_raw",
            "Sony WH-1000XM5 India price drop authorised dealer news\norganic protein bar bundle Mumbai BigBasket modern trade", true)}
...
function OnboardingChat({
  ...
  useEffect(() => {
    fetch(`${API}/sales-deals/${projectId}/config`)
  ...
  useEffect(() => {
    const ws = new WebSocket(`${WS_API}/sales-deals/ws/onboard?project_id=${projectId}`);
    wsRef.current = ws;
    ...
    <div className="h-full flex flex-col max-w-2xl mx-auto px-6 py-8">
      ...
      <div className="mb-5">
        <div className="flex items-center justify-between mb-1">
          <h1 className="text-xl font-semibold text-ui-text">Set up your Sales Deal Agent</h1>
          <button
...
function DetailPanel({ lead, onClose, onSave, onDelete }: {
  ...
  const [nameValue, setNameValue]     = useState(dealDisplayName(lead));
  ...
  useEffect(() => {
    ...
    setNameValue(dealDisplayName(lead));
  }, [lead]);
  ...
  const commitName = () => {
    ...
    if (!trimmed || trimmed === dealDisplayName(lead)) { setEditingName(false); return; }
    ...
    <div className="absolute inset-y-0 right-0 w-[480px] bg-ui-card border-l border-ui-border z-20 overflow-y-auto flex flex-col">
      <div className="flex items-center justify-between p-5 border-b border-ui-border">
        {editingName ? (
          <input
            ...
            onBlur={commitName}
            onKeyDown={e => { if (e.key === "Enter") commitName(); if (e.key === "Escape") { setNameValue(dealDisplayName(lead)); setEditingName(false); } }}
          ...
          >
            <span className="truncate">{dealDisplayName(lead)}</span>
      ...
      <div className="flex-1 p-5 space-y-5">
        ...
        <div>
          ...
          {isPlaceholderDealUrl(lead.source_url) && (
            <p className="text-[11px] text-amber-400 mb-2 rounded-lg border border-amber-900/60 bg-amber-950/40 px-2 py-1.5">
              Placeholder URL (mock search). Configure SERPER_API_KEY for real listing links.
            </p>
          )}
...
function CorrectLeadsChat({
  ...
  useEffect(() => {
    ...
    const ws = new WebSocket(`${WS_API}/sales-deals/ws/correct?project_id=${projectId}`);
    wsRef.current = ws;
      ...
      {!revised && (
        <div className="px-6 pb-6 max-w-2xl w-full mx-auto flex gap-2">
          <input
            ...
            onChange={e => setInput(e.target.value)}
            onKeyDown={e => e.key === "Enter" && !e.shiftKey && send()}
            placeholder={wsReady ? "Describe what's wrong with the current deals…" : "Connecting…"}
...
function EditConfigPanel({
  ...
  const [saving, setSaving]   = useState(false);
  const [err, setErr]         = useState("");
  const schema = context.result_schema ?? { lead_name_field: "product_name", categories: [], geographies: [], signal_types: [] };
  const [form, setForm]       = useState({
    ...
    product_focus:         (cfg as { product_focus?: string }).product_focus ?? "",
    ...
    search_channels:       [...(context.search_channels ?? ["google", "news"])],
    ...
    lead_name_field:       schema.lead_name_field ?? "product_name",
  ...
  const handleSave = async () => {
    ...
    if (!form.product_focus.trim() && !form.ideal_customer_profile.trim()) {
      setErr("Set \"What deals you’re looking for\" (product focus) or region/buyer context.");
      return;
    }
    ...
    const result_schema = {
      lead_name_field: form.lead_name_field || "product_name",
    ...
    const payload = {
      ...
      search_queries:        search_queries.length ? search_queries : [{ signal: "general", queries: [
        `${form.product_focus.trim() || "product"} ${form.search_geo || "India"} price news authorised retailer`,
      ] }],
      search_channels:       (form.search_channels as string[]).length ? form.search_channels : ["google", "news"],
        ...
        <div className="max-w-3xl mx-auto space-y-8">
          ...
          <section>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Agent name" k="agent_name" placeholder="My Sales Deal Agent" />
          ...
          <section>
            <p className="text-[11px] uppercase tracking-wider text-ui-accent font-semibold mb-3">Targeting</p>
            <div className="space-y-3">
              <Field label="What deals you’re looking for" k="product_focus"
                placeholder="SKU/category, price band, region (e.g. vacuum cleaner under ₹15k, India online)" multiline />
              <Field label="Qualifier context" k="qualifier_context"
                placeholder="How to judge if a snippet is a real deal matching the focus above." multiline />
              <Field label="Region / buyer context" k="ideal_customer_profile"
                placeholder="Where and for whom these deal alerts matter." multiline />
              <Field label="What we do with hits" k="what_we_offer"
                placeholder="e.g. alert field team, track MAP, compare marketplaces" multiline />
            </div>
          </section>
          ...
          {/* Result schema */}
          <section>
            <p className="text-[11px] uppercase tracking-wider text-ui-accent font-semibold mb-3">Deal classification</p>
            <p className="text-[11px] text-ui-accent mb-3">These fields define how the AI labels each deal. Keep them consistent with your hunt criteria above.</p>
            <div className="space-y-3">
              <div>
                <label className="block text-xs font-medium text-ui-accent mb-1">Primary field (e.g. product_name)</label>
                <select value={form.lead_name_field} onChange={e => set("lead_name_field", e.target.value)}
                  className="w-full bg-ui-bg border border-ui-border rounded-lg px-3 py-2 text-sm text-ui-text">
                  <option value="product_name">product_name</option>
                  <option value="company_name">company_name (retailer)</option>
...
function ContextInspector({ context }: { context: AgentContext }) {
  ...
  return (
    <div className="space-y-5">
        ...
        <div className="col-span-1 bg-ui-card border border-ui-border rounded-xl p-5 flex flex-col gap-3">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              <p className="font-semibold text-ui-text truncate">{context.config.agent_name || "Sales Deal Agent"}</p>
      ...
      {/* ── Row 2: Hunt criteria · ICP · What we offer ── */}
      {(context.config.product_focus || context.config.qualifier_context || context.config.ideal_customer_profile || context.config.what_we_offer) && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {context.config.product_focus && (
            <div className="bg-ui-card border border-violet-900/50 rounded-xl p-4">
              <p className="text-[10px] uppercase tracking-wider text-violet-300 font-semibold mb-2">What you’re looking for</p>
              <p className="text-xs text-ui-text leading-relaxed">{context.config.product_focus}</p>
            </div>
          )}
          ...
          {context.config.ideal_customer_profile && (
            <div className="bg-ui-card border border-ui-border rounded-xl p-4">
              <p className="text-[10px] uppercase tracking-wider text-blue-400 font-semibold mb-2">Region / buyer context</p>
...
function RefineQueriesPanel({
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch(`${API}/sales-deals/${projectId}/refine-queries`, { method: "POST" });
        if (!r.ok) {
          const e = await r.json().catch(() => ({}));
  ...
  const handleApply = async () => {
    ...
    setApplying(true);
    try {
      const r = await fetch(`${API}/sales-deals/${projectId}/refine-queries/apply`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
function Dashboard({
  ...
  const fetchStats = useCallback(async () => {
    const r = await fetch(`${API}/sales-deals/${pid}/stats`);
    if (r.ok) setStats(await r.json());
  }, [pid]);

  const fetchLeads = useCallback(async () => {
    const r = await fetch(`${API}/sales-deals/${pid}/deals`);
    if (r.ok) setLeads(await r.json());
  }, [pid]);

  const fetchLog = useCallback(async () => {
    const r = await fetch(`${API}/sales-deals/${pid}/log`);
    if (r.ok) { const d = await r.json(); setLog(d.log); }
  }, [pid]);

  const fetchContext = useCallback(async () => {
    const r = await fetch(`${API}/sales-deals/${pid}/context`);
    if (r.ok) setAgentContext(await r.json());
  }, [pid]);

  const fetchStatus = useCallback(async () => {
    const r = await fetch(`${API}/sales-deals/${pid}/status`);
    if (r.ok) setJobStatus(await r.json());
  }, [pid]);

  const fetchSearchHistory = useCallback(async () => {
    const r = await fetch(`${API}/sales-deals/${pid}/search-history`);
    if (r.ok) { const d = await r.json(); setSearchHistoryCount(d.total ?? 0); }
  }, [pid]);

  const clearSearchHistory = async () => {
    await fetch(`${API}/sales-deals/${pid}/search-history`, { method: "DELETE" });
  ...
  const handleSave = useCallback(async (id: string, patch: Partial<Lead>) => {
    const r = await fetch(`${API}/sales-deals/${pid}/deals/${id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
  ...
  const handleDelete = useCallback(async (id: string) => {
    await fetch(`${API}/sales-deals/${pid}/deals/${id}`, { method: "DELETE" });
  ...
  const triggerRun = async () => {
    const r = await fetch(`${API}/sales-deals/${pid}/run`, { method: "POST" });
    if (r.ok) {
  ...
  const triggerStop = async () => {
    await fetch(`${API}/sales-deals/${pid}/stop`, { method: "POST" });
  ...
  const triggerCleanup = async () => {
    ...
    fetchStatus();
    const r = await fetch(`${API}/sales-deals/${pid}/cleanup`, { method: "POST" });
    if (r.ok) {
      const s = await r.json();
  ...
  const displayed = leads.filter(l => {
    ...
    if (search.trim()) {
      ...
      const name = asLower(dealDisplayName(l));
  ...
  const downloadCsv = useCallback(() => {
    ...
    const rows = [cols.join(","), ...displayed.map(l =>
      cols.map(c => escape(c === "brand_name" ? dealDisplayName(l) : l[c as keyof Lead])).join(",")
    )];
  ...
  return (
    <div className="h-full flex overflow-hidden bg-ui-bg text-ui-text text-sm relative">
            ...
            {[50, 100, 200, 0].map(n => {
              ...
              return (
                <button
                  key={n}
                  onClick={async () => {
                    await fetch(`${API}/sales-deals/${pid}/config`, {
                      method: "PATCH",
                      headers: { "Content-Type": "application/json" },
      ...
      <main className="flex-1 overflow-y-auto p-6">
        ...
        {stats.mock_search && (
          <div className="mb-4 rounded-xl border border-amber-800/80 bg-amber-950/50 px-4 py-3 text-sm text-amber-100/95">
            <p className="font-semibold text-amber-50 mb-1">Mock search — links are placeholders</p>
            <p className="text-amber-100/90 leading-relaxed">
              There is no <code className="rounded bg-black/30 px-1 py-0.5 text-xs">SERPER_API_KEY</code> in your environment.
              The agent simulates one result per query so you can test scoring; URLs like{" "}
              <code className="text-xs">example.com/mock-deal/…</code> are not real pages.
              Add a key from{" "}
              <a href="https://serper.dev" target="_blank" rel="noreferrer" className="text-amber-300 underline hover:text-amber-200">serper.dev</a>
              {" "}to <code className="rounded bg-black/30 px-1 py-0.5 text-xs">sales-deal-agent/.env</code>, restart the API, use{" "}
              <span className="font-medium">Re-search all</span> (or delete <code className="text-xs">search_history.json</code>), then run the agent again for real retailer links.
            </p>
          </div>
        )}
                    ...
                    {displayed.map(lead => (
                      <tr key={lead.id} onClick={() => setSelected(lead)}
                        ...
                        <td className="px-3 py-2.5 font-semibold text-ui-text max-w-[200px]">
                          <span className="flex items-center gap-1 truncate" title={dealDisplayName(lead)}>
                            {lead.starred && <Star className="w-3 h-3 text-yellow-400 fill-yellow-400 flex-shrink-0" />}
                            {dealDisplayName(lead)}
                          </span>
...
export default function SalesDealTool() {
  ...
  const handleSelectProject = async (project: Project) => {
    ...
    try {
      const r = await fetch(`${API}/sales-deals/${project.id}/config`);
      if (r.ok) {
  ...
  const handleDeleteProject = async (pid: string) => {
    try {
      const r = await fetch(`${API}/sales-deals/projects/${pid}`, { method: "DELETE" });
      if (r.ok) {
  ...
  const handleRenameProject = async (pid: string, name: string) => {
    try {
      const r = await fetch(`${API}/sales-deals/projects/${pid}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name }),
      });
